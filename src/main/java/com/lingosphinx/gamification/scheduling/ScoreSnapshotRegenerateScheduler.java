// src/main/java/com/lingosphinx/gamification/scheduling/ScoreSnapshotRegenerateScheduler.java
...
public class ScoreSnapshotRegenerateScheduler {
    ...
    private void scheduleForTimeZone(IanaTimeZone tz) {
        ...
        taskScheduler.scheduleAtFixedRate(() -> {
            log.info("Midnight in {}: executing regeneration", tz.zoneId());
            try {
                scoreSnapshotService.regenerateAll(tz);
            }
            catch (Throwable t) {
                log.error("Error during score snapshot regeneration for timezone {}", tz.zoneId(), t);
            }
